...
// FIX : Animation optionnelle, non bloquante pour l'affichage des produits
...
const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            // Ajouter une classe d'animation légère (optionnel)
            entry.target.classList.add('fade-in');
            // Ne plus observer après la première apparition
            observer.unobserve(entry.target);
        }
    });
}, observerOptions);

// Observer uniquement les sections (pas les cartes individuelles)
// Les cartes produits sont maintenant toujours visibles
